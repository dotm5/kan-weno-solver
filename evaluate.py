...
def run_evaluation(model_path='kan_model.pth'):
    ...
    while t < T_final:
        ...
        for _ in range(ref_substeps):
            u_truth = rk3_step(u_truth, dx_ref, dt / ref_substeps, nu=0.0)
        u_coarse = rk3_step(u_coarse, dx_coarse, dt, nu=0.0)
        u_phys = rk3_step(u_hybrid, dx_coarse, dt, nu=0.0)

        corr, _ = predictor.predict(u_hybrid, dx_coarse, dt, t_stamp=t)
